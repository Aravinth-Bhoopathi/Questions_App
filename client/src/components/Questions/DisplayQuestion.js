...
import { startUpdateQuestion } from "../../action/questionAction";
    ...
    return (
        <div style={{position:'relative'}}>
            <div className="question_left">
                <h5 className="text-info">My Questions</h5>
                {userQuestion.map((ele,i)=>{
                    return (
                        <div key={ele._id} className='user_questions'>
                        <p> {i+1} : {ele.data}</p>
                        <button onClick={()=>{handleLike(ele._id)}} disabled >Likes-{ele.likes.length}</button>
            ...
            <div className="question_right">
                <h5 className="text-info">Others Questions</h5>
                {allQuestion.map((ele,i)=>{ 
                    return (
                        <div key={ele._id} className='user_questions'> 
                        <p> {i+1} : {ele.data}</p>
                        <button onClick={()=>{handleLike(ele._id)}} >Likes-{ele.likes.length}</button>
                        <span>~{ele.userId.name}</span>
                        </div>
                    )
                })}
